(function(){
  ...
  const timezoneSelect = document.getElementById('bkTimezone');
  ...
  const altRequestLink = document.getElementById('bkAltRequest');
    ...
    if (timezoneSelect){
      timezoneSelect.addEventListener('change', ()=>{
        if (selectedFullDate && selectedFullLabel){
          selectedFullLabel.textContent = formatFullDateLabel(selectedFullDate, timezoneSelect.value);
        }
      });
    }
  ...
  if (backButton){
    backButton.addEventListener('click', ()=>{
      ...
      const grid = document.querySelector('.bk-page');
      ...
      document.body.classList.remove('bk-fullview-active');
  ...
  function renderTimesForFull(iso){
    ...
    if (selectedFullLabel){
      const tz = timezoneSelect ? timezoneSelect.value : 'America/Toronto';
      selectedFullLabel.textContent = formatFullDateLabel(iso, tz);
    }
    if (continueFull){
      continueFull.dataset.date = iso;
  ...
  function formatFullDateLabel(iso, tz){
