...
(function(){
  ...
  function readFolderListing(folder){
    const jsonUrl = encodeURI(toUrl(folder.replace(/\/$/,'') + '/_images.json'));
    ...
    async mount(){
      ...
      if (Array.isArray(images) && images.length){
        ...
        if (inline){
          ...
          // If inline paths reference the logical Images/ folder, remap them for device (Desktop vs Mobile)
          if (Array.isArray(list) && list.some(it => typeof it === 'string' && /Images(\/|$)/.test(it))){
            const isMobileInline = (window.matchMedia && window.matchMedia('(max-width: 900px)').matches);
            list = list.map(item => {
              if (typeof item !== 'string') return item;
              if (/Images-(Desktop|Mobile)/.test(item)) return item;
              return item.replace(/Images(\/|$)/, (isMobileInline ? 'Images-Mobile$1' : 'Images-Desktop$1'));
            });
          }
        }
        if (!list.length && src){
          // Map logical Images/ -> Images-Desktop/ or Images-Mobile/ based on screen size
          const isMobile = (window.matchMedia && window.matchMedia('(max-width: 900px)').matches);
          let resolvedSrc = src;
          if (!/Images-(Desktop|Mobile)/.test(resolvedSrc)){
            resolvedSrc = resolvedSrc.replace(/Images(\/|$)/, (isMobile ? 'Images-Mobile$1' : 'Images-Desktop$1'));
          }
          list = await readFolderListing(resolvedSrc);
        }
      }
      if (!list.length){ console.warn('FXCarousel: no images source'); return; }

      // If a sample size is requested, pick up to that many images once per page load
      const sampleAttr = Number(this.root.getAttribute('data-sample')) || 15;
      let working = list;
      if (Array.isArray(working) && working.length > sampleAttr){
        working = sampleArray(working, sampleAttr);
      }
      // Trust provided lists; skip verifying every image to avoid heavy memory/CPU on mobile
      this.state.imgs = doShuffle ? shuffle(working.slice()) : working;
