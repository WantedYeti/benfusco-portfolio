  ...
  this.applyAspect();
    ...
    crossfadeHint(via){
      ...
      requestAnimationFrame(() => {
        ...
        // Handle seamless wrap using clones to avoid long jumps and black frames
        if (this.opts.loop && via === 'next' && before === N-1 && next === 0){
          const { track } = this.ui;
          // Animate from last real (j=N) to trailing clone (j=N+1), then snap to first real (j=1)
          const jFrom = before + this.state.offset; // N
          const jClone = jFrom + 1; // N+1
          // Ensure we have layout for tight mode
          if (this.opts.tight && !this.state.widths.length) { this.computeLayout(); }
          // Start from current position (should already be at jFrom), then animate to clone
          track.style.transition = `transform 620ms ${ease}`;
