...
(function(){
  ...
  function initGridLightbox(){
    ...
    function open(i){
      ...
      setBodyLock(true);
      // Pause all currently playing carousels and remember which to resume
      try {
        pausedCarousels = [];
        const carousels = Array.from(document.querySelectorAll('.fx-carousel'))
          .map(n => n.__fx)
          .filter(Boolean);
        carousels.forEach(inst => {
          if (inst?.opts?.autoplay?.enabled && inst?.state?.playing && !inst?.state?.userPaused){
            pausedCarousels.push(inst);
          }
          inst.pause();
        });
      } catch(_){}
    ...
    function close(){
      ...
      // Resume only the carousels that were playing before open
      try {
        pausedCarousels.forEach(inst => inst.play());
        pausedCarousels = [];
      } catch(_){}
    }
    ...
    // Also close when clicking inside inner area but not on the image or controls/dots
    ui.inner.addEventListener('click', (e) => {
      const clickedImg = e.target.closest('.fx-lb-img');
      const clickedControl = e.target.closest('.fx-lb-prev, .fx-lb-next, .fx-lb-close, .fx-dots, .fx-dot');
      if (!clickedImg && !clickedControl) close();
    });
