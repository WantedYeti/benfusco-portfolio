...
(function(){
  ...
  function autoInit(){
    ...
    nodes.forEach(node => {
      ...
      if (imagesAttr) {
        try { images = JSON.parse(imagesAttr); } catch(e){ console.warn('FXCarousel: invalid data-images JSON', e); }
      }
      const delayAttr = node.getAttribute('data-delay');
      const delay = delayAttr ? parseInt(delayAttr, 10) : undefined;
      node.__fx = new FXCarousel(node, {
        src: node.getAttribute('data-src') || undefined,
        images,
        shuffle: (node.getAttribute('data-shuffle')||'false') === 'true',
        autoplay: { enabled: true, delay: delay || 3200 }
      });
    });
  }
