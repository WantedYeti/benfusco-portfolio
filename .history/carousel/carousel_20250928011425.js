...
(function(){
  ...
  const DUR = 740; // ms, match CSS for smoother, consistent timing
  ...
  this.state = { idx: 0, timer: null, playing: true, imgs: [], ready: false, widths: [], prefix: [], offset: 0, animating: false, needResize: false };
      this.bound = {
    ...
    async mount(){
      ...
      // Decode left/center/right before first paint to avoid blank neighbor on refresh
      try {
        const N = this.state.imgs.length;
        const left = this.opts.loop && N>1 ? this.state.imgs[N-1] : this.state.imgs[0];
        const center = this.state.imgs[0];
        const right = this.state.imgs[Math.min(1, N-1)];
        await Promise.race([
          Promise.all([decodeOnce(left), decodeOnce(center), decodeOnce(right)]),
    ...
    renderSlides(){
      ...
      const make = (src, i) => {
        ...
        img.alt = '';
        img.decoding = 'async';
        // Eagerly load first, previous, and next (including clones -1 and N)
        const eager = (i === -1 || i === 0 || i === 1 || i === imgs.length);
    ...
    bind(){
      ...
      nextBtn.addEventListener('click', () => { this.bound.next(); });
    ...
    go(targetIdx, meta={}){
      ...
      if (this.opts.loop && via === 'next' && before === N-1 && next === 0){
        const { track } = this.ui;
        const jFrom = before + this.state.offset; // N
        const jClone = jFrom + 1; // N+1
        if (this.opts.tight && !this.state.widths.length) { this.computeLayout(); }
        this.state.animating = true;
        track.style.transition = `transform ${DUR}ms ${ease}`;
        requestAnimationFrame(() => { track.style.transform = `translate3d(${this.baseFor(jClone)}px,0,0)`; });
        const onEnd = () => {
