...
(function(){
  ...
  class FXCarousel {
    constructor(root, options={}){
          // True lazy load: set data-src and let IO/neighbors trigger actual load
          img.loading = 'lazy';
  ...
  this.state = { idx: 0, timer: null, playing: true, userPaused: false, imgs: [], ready: false, widths: [], prefix: [], offset: 0, animating: false, needResize: false };
    ...
    this.attachLazy();
    // Ensure current and neighbor slides are loaded immediately to avoid blanks
    this.ensureNeighborsLoaded(2);
      ...
      try {
        ...
        // Make sure decode targets are loading
        this.ensureNeighborsLoaded(2);
        const j = this.state.idx + this.state.offset; // current absolute index in track
        const decodeTargets = [j-2, j-1, j, j+1, j+2]
          .map(k => imgsInTrack[k])
          .filter(Boolean);
        await Promise.race([
      ...
      this.preloadAhead();
      updatePreloads([this.state.imgs[(this.state.idx+1)%this.slideCount()], this.state.imgs[(this.state.idx+2)%this.slideCount()]]);
    ...
    attachLazy(){
      ...
      if ('IntersectionObserver' in window){
        ...
        // Fallback: load all if IO unsupported
    ...
    renderSlides(){
      ...
      const make = (src, i) => {
        ...
        img.alt = '';
        img.decoding = 'async';
        // Eagerly load previous clone, first, next1, next2, and trailing clone
