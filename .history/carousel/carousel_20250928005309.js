  ...
  this.applyAspect();
    ...
    renderSlides(){
      ...
      const make = (src, i) => {
        ...
        const onLoad = () => {
          ...
          // Compute base translateX for an absolute child index (including clones)
          baseFor(jAbs){
          ...
          applyTransform(immediate){
            ...
            const base = this.baseFor(j);
            track.style.transform = `translate3d(${base}px,0,0)`;
            // Update classes using a simple heuristic for neighbors
            const viewportW = this.ui.viewport.clientWidth;
    ...
    updateClasses(perView){
      ...
      go(targetIdx, meta={}){
        ...
        // Seamless wrap using clones to avoid black/phantom frames
        if (this.opts.loop && via === 'next' && before === N-1 && next === 0){
          const { track } = this.ui;
          const jFrom = before + this.state.offset; // last real (N)
          const jClone = jFrom + 1; // trailing clone (N+1)
          if (this.opts.tight && !this.state.widths.length) { this.computeLayout(); }
          // Animate to clone
          track.style.transition = `transform 620ms ${ease}`;
          track.style.transform = `translate3d(${this.baseFor(jClone)}px,0,0)`;
          const onEnd = () => {
            track.removeEventListener('transitionend', onEnd);
            // Snap to first real without animation
            track.style.transition = 'none';
            this.state.idx = 0;
            track.style.transform = `translate3d(${this.baseFor(1)}px,0,0)`;
            ...
            if (inImg) { inImg.classList.add('fade-in'); requestAnimationFrame(()=>inImg.classList.add('fading')); setTimeout(()=>inImg.classList.remove('fade-in','fading'), 520); }
        ...
        if (this.opts.loop && via === 'prev' && before === 0 && next === N-1){
          const { track } = this.ui;
          const jFrom = before + this.state.offset; // first real (1)
          const jClone = jFrom - 1; // leading clone (0)
          if (this.opts.tight && !this.state.widths.length) { this.computeLayout(); }
          // Animate to clone
          track.style.transition = `transform 620ms ${ease}`;
          track.style.transform = `translate3d(${this.baseFor(jClone)}px,0,0)`;
          const onEnd = () => {
            ...
            track.style.transition = 'none';
            this.state.idx = N-1;
            track.style.transform = `translate3d(${this.baseFor(N)}px,0,0)`;
            this.updateClasses(1);
            void track.getBoundingClientRect();
            track.style.transition = `transform 620ms ${ease}`;
            // No schedule here; prev usually comes from manual input that already paused
          };
          track.addEventListener('transitionend', onEnd);
          return;
        }
      ...
      computeLayout(){
        ...
        this.state.prefix = prefix;
            ...
            track.style.transform = `translate3d(${this.baseFor(1)}px,0,0)`;
            this.updateClasses(1);
            // Force reflow and restore transition for subsequent moves
            void track.getBoundingClientRect();
            track.style.transition = `transform 620ms ${ease}`;
            this.schedule();
            this.preloadAhead();
        ...
        if (this.opts.loop && via === 'prev' && before === 0 && next === N-1){
          const { track } = this.ui;
          // Animate from first real (j=1) to leading clone (j=0), then snap to last real (j=N)
          const jFrom = before + this.state.offset; // 1
          const jClone = jFrom - 1; // 0
          if (this.opts.tight && !this.state.widths.length) { this.computeLayout(); }
          track.style.transition = `transform 620ms ${ease}`;
          track.style.transform = `translate3d(${this.baseFor(jClone)}px,0,0)`;
          const onEnd = () => {
            ...
            void track.getBoundingClientRect();
            track.style.transition = `transform 620ms ${ease}`;
            // Do not auto-resume since prev usually pauses already in handler; keep schedule consistent
          };
          track.addEventListener('transitionend', onEnd);
          return;
        }
    ...
    schedule(){
        // For normal moves, rely on transform animation only
      const children = Array.from(this.ui.track.children);
      const widths = [];
      for (const slide of children){
        const img = slide.querySelector('img');
