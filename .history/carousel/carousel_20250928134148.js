...
(function(){
  ...
  this.crossfadeHint(true);
      ...
      const onEnd = () => {
        ...
        // Snap absolute position back into middle block to avoid drift
        if (this.opts.loop) {
          const N = this.slideCount();
          // Keep the absolute index anchored around the middle (offset = N)
          this.state.offset = N;
          track.style.transition = 'none';
          track.style.transform = `translate3d(${this.baseFor(this.state.idx + this.state.offset)}px,0,0)`;
