...
(function(){
  ...
  const DUR = 800; // ms, consistent smooth duration per request
  ...
  this.applyAspect();
      ...
      try {
        ...
        const decodeTargets = [j-5, j-4, j-3, j-2, j-1, j, j+1, j+2, j+3, j+4, j+5]
          .map(k => imgsInTrack[k])
          .filter(Boolean);
  ...
  track.style.transform = `translate3d(${base}px,0,0)`;
      ...
      if (immediate){
        ...
        cancelAnimationFrame(this._rafTransform);
        this._rafTransform = requestAnimationFrame(() => {
          setX();
        });
      }
  const viewportW = this.ui.viewport.clientWidth;
