...
(function(){
  ...
  class FXCarousel {
    constructor(root, options={}){
      ...
      // Warm up neighbors for the very first wrap
      this.preloadAhead();
      ...
      const make = (src, i) => {
        ...
        img.alt = '';
        img.decoding = 'async';
        // Eagerly load loop-edge clones to avoid a brief blank when wrapping
        const isClone = (i < 0) || (i >= this.state.imgs.length);
        img.loading = isClone ? 'eager' : 'lazy';
        if (isClone) {
      ...
      const checks = list.map(src => new Promise(resolve => {
        ...
        return results.map(src => ({ src, w: img.naturalWidth || 0, h: img.naturalHeight || 0 }));
    }
    ...
    renderSlides(){
      ...
      if (isClone) {
        ...
        img.addEventListener('click', this.bound.openLightbox);
        slide.appendChild(img);
          // If we already know aspect, mark vertical immediately to avoid black bg on first paint
          if (entry && typeof entry === 'object' && entry.w && entry.h) {
            const r = entry.w / entry.h;
            slide.dataset.r = String(r);
            if (entry.h > entry.w * 1.05) {
              slide.classList.add('is-vertical');
              slide.style.setProperty('--fx-bg', `url("${src}")`);
            }
          }
    ...
    applyTransform(immediate, jOverride){
      const { track } = this.ui;
    ...
    track.style.transition = immediate ? 'none' : `transform 620ms ${ease}`;
      if (this.opts.tight && this.state.widths.length) {
        const j = (typeof jOverride === 'number') ? jOverride : (this.state.idx + this.state.offset);
        const before = this.state.prefix[j] || 0; // sum of widths before j
        const curW = this.state.widths[j] || 0;
        const center = Math.round(before + curW / 2);
        let base = Math.round((viewportW / 2) - center); // integer pixel to avoid subpixel shimmer
        // Clamp so we never expose empty viewport beyond the track edges
        const widths = this.state.widths;
        const last = widths.length - 1;
        const totalW = (this.state.prefix[last] || 0) + (widths[last] || 0);
        base = Math.min(0, Math.max(base, viewportW - totalW));
        track.style.transform = `translate3d(${base}px,0,0)`;
        this.updateClasses(3, j); // keep class state aligned with the j we centered
      } else {
        const perView = viewportW < 900 ? this.opts.perViewMobile : this.opts.perViewDesktop;
        const slideW = viewportW / perView;
      const j = (typeof jOverride === 'number') ? jOverride : (this.state.idx + this.state.offset);
      let base = -Math.round(j * slideW);
        const totalW = slideW * this.ui.track.children.length;
        base = Math.min(0, Math.max(base, viewportW - totalW));
        track.style.transform = `translate3d(${base}px,0,0)`;
        this.updateClasses(perView, j);
      }
    }
    ...
    updateClasses(perView, jOverride){
      const slides = this.ui.track.children;
      const activeAt = (typeof jOverride === 'number') ? jOverride : (this.state.idx + this.state.offset);
    ...
    go(targetIdx, meta={}){
      ...
      this.state.idx = next;
      // If wrapping, animate to the virtual clone to avoid jumping across the track
      let jOverride = undefined;
      if (this.opts.loop){
        if (before === N-1 && next === 0 && via === 'next'){
          // trailing clone sits after last real: index = (N + 1) in children
          jOverride = this.state.offset + N; // 1 + N
        } else if (before === 0 && next === N-1 && via === 'prev'){
          // leading clone sits at index 0 in children
          jOverride = 0;
        }
      }
      this.applyTransform(false, jOverride);
    // Fade in the newly active slide image for a smoother feel (skip on wrap to clones)
    const wrappingForward = (this.opts.loop && before === N-1 && next === 0 && via === 'next');
    const wrappingBackward = (this.opts.loop && before === 0 && next === N-1 && via === 'prev');
    if (!wrappingForward && !wrappingBackward) {
      ...
      if (this.opts.loop){
        ...
        // Ensure no transient fade classes linger after snapping
        const imgs = track.querySelectorAll('img.fx-img.fade-in, img.fx-img.fading, img.fx-img.fade-out');
        imgs.forEach(el => el.classList.remove('fade-in','fading','fade-out'));
        };
