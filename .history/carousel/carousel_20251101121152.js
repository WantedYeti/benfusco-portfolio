...
(function(){
  ...
  prevBtn.addEventListener('click', () => {
    ...
    baseFor(jAbs){
      const viewportW = Math.max(1, this.ui.viewport.clientWidth);
      if (this.opts.tight && this.state.widths.length){
        const widths = this.state.widths;
        const prefix = this.state.prefix;
        const maxIdx = widths.length - 1;
        const idx = Math.min(Math.max(0, jAbs), maxIdx);
        const slideStart = prefix[idx] || 0;
        const slideWidth = widths[idx] || (viewportW / 3);
        const center = slideStart + (slideWidth / 2);
      ...
      const slideW = perView > 0 ? viewportW / perView : viewportW;
      const center = (jAbs + 0.5) * slideW;
      return Math.round((viewportW / 2) - center);
    }
