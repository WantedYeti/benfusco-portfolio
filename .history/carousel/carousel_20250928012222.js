...
(function(){
  ...
  this.state = { idx: 0, timer: null, playing: true, userPaused: false, imgs: [], ready: false, widths: [], prefix: [], offset: 0, animating: false, needResize: false };
    ...
    async mount(){
      ...
      try {
        const els = this.ui.track.querySelectorAll('img.fx-img');
        const leftEl = els[0];
        const centerEl = els[1] || els[0];
        const rightEl = els[2] || centerEl;
        await Promise.race([
          Promise.all([decodeEl(leftEl), decodeEl(centerEl), decodeEl(rightEl)]),
  ...
  prevBtn.addEventListener('click', () => { this.bound.prev(); });
